...
import { FilePen, Trash2, Plus, Check, Loader2, AlertTriangle, Layers } from "lucide-react";
...
export default function DevisUpdateForm({ data, onCreated }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (lignesValides.length === 0) { toast.error("Aucune ligne à ajouter"); return; }

    setSaving(true);
    try {
      // Récupère le max d'ordre existant
      ...
      await supabase
        .from("devis")
        .update({ montant_ht: Math.round(newMontantHT * 100) / 100 } as any)
        .eq("id", data.devis_id);

      toast.success(`${lignesValides.length} ligne${lignesValides.length > 1 ? "s" : ""} ajoutée${lignesValides.length > 1 ? "s" : ""} au devis !`);
      onCreated();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-3 my-2 animate-fade-up">
      ...
      <Card className="border-blue-500/30 bg-blue-500/5">
        <CardHeader className="py-2 px-3">
          <CardTitle className="text-xs flex items-center gap-1.5">
            ...
            Ajout de lignes — devis{data.devis_numero ? ` ${data.devis_numero}` : ""} (brouillon)
          </CardTitle>
        </CardHeader>
      ...
      <Button onClick={handleSubmit} disabled={saving || !data.devis_id} className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white">
        {saving
          ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Mise à jour…</>
          : <><Check className="w-4 h-4 mr-2" /> Ajouter au devis</>
        }
      </Button>
    </div>
  );
