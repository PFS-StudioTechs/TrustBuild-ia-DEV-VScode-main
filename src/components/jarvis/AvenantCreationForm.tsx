...
export function parseAvenantData(text: string): AvenantData | null {
  const match = text.match(/<!--AVENANT_DATA\s*([\s\S]*?)\s*AVENANT_DATA-->/);
  if (!match) return null;
  try { return JSON.parse(match[1]); } catch { return null; }
}

export function stripAvenantData(text: string): string {
  return text.replace(/<!--AVENANT_DATA[\s\S]*?AVENANT_DATA-->/g, "").trim();
}

interface Props {
  data: AvenantData;
  onCreated: (avenantId: string) => void;
}
...
export default function AvenantCreationForm({ data, onCreated }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (!motif.trim()) { toast.error("Le motif de l'avenant est obligatoire"); return; }

    setSaving(true);
    try {
      const { data: settingsRow } = await supabase
        .from("artisan_settings").select("avenant_prefix").eq("user_id", user.id).maybeSingle();
      const prefix = settingsRow?.avenant_prefix?.trim() || "AVN";
      const numero = `${prefix}-${Date.now().toString(36).toUpperCase()}`;

      const { data: newAvenant, error: avErr } = await supabase
        .from("avenants")
        .insert({
          artisan_id: user.id,
      ...
      if (avErr) throw new Error(`Avenant: ${avErr.message}`);

      const lignesValides = lignes.filter(l => l.description.trim() || l.prix_unitaire > 0);
      if (lignesValides.length > 0) {
        await (supabase as any).from("lignes_avenant").insert(
      ...
      <Card className="border-amber-500/30 bg-amber-500/5">
        <CardHeader className="py-2 px-3">
          <CardTitle className="text-xs flex items-center gap-1.5">
            ...
            Avenant{data.devis_numero ? ` — devis ${data.devis_numero}` : ""}
          </CardTitle>
        </CardHeader>
        <CardContent className="px-3 pb-3 space-y-2">
          <div>
            <Label className="text-[10px]">Motif de l'avenant *</Label>
            <Input value={motif} onChange={e => setMotif(e.target.value)} className="h-8 text-xs" placeholder="Ex : Travaux supplémentaires demandés par le client" />
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-muted">
        ...
        <CardContent className="px-3 pb-3 space-y-2">
          ...
          <div className="flex justify-between items-center pt-2 border-t text-xs font-semibold">
            <span>Total HT avenant</span>
            <span className={totalHT >= 0 ? "text-primary" : "text-destructive"}>
              {totalHT >= 0 ? "+" : ""}{totalHT.toLocaleString("fr-FR", { style: "currency", currency: "EUR" })}
            </span>
          </div>
        </CardContent>
      </Card>

      <Button onClick={handleSubmit} disabled={saving || !data.devis_id} className="w-full bg-gradient-to-r from-amber-500 to-orange-500 text-white">
        {saving ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Création…</> : <><Check className="w-4 h-4 mr-2" /> Créer l'avenant</>}
      </Button>
    </div>
  );
}
