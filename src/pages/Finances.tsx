            ...
            chantierStats.map(ch => (
              <div key={ch.id} className="forge-card">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
                  <div><p className="text-xs text-muted-foreground">Budget ajusté</p><p className="font-mono font-semibold text-foreground">{Number(ch.budgetDevis).toLocaleString("fr-FR")} €</p></div>
          ...
          <div className="forge-card">
            ...
            <p className="text-sm text-muted-foreground">
              Basé sur les devis signés en cours : <span className="font-mono font-semibold text-primary">
                {devis.filter(d => d.statut === "signe").reduce((s, d) => {
                  const totalAvenants = avenants.filter(a => a.devis_id === d.id).reduce((sum, a) => sum + Number(a.montant_ht), 0);
                  const totalAvoirs = avoirs.filter(a => a.devis_id === d.id).reduce((sum, a) => sum + Number(a.montant_ht), 0);
                  return s + Number(d.montant_ht) + totalAvenants - totalAvoirs;
