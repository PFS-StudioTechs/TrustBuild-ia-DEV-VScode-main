...
import { Loader2, Check, X, Edit3, RotateCcw, Strikethrough, CircleDot, CreditCard } from "lucide-react";
...
export default function DocumentPublic() {
    ...
    fetch(`${SUPABASE_URL}/functions/v1/devis-public?token=${token}`, {
      ...
      .then(d => {
        if (d.expired) { setExpired(true); setErr(d.error); }
          ...
          setData(d);
          if (d.doc?.statut === "signe") setFinalStatus("signe");
          if (d.doc?.statut === "refuse") setFinalStatus("refuse");
        }
      })
  ...
  function toggleAnn(lineId: string, key: keyof LineAnn) {
    setLineAnns(prev => { const cur = prev[lineId] ?? {}; return { ...prev, [lineId]: { ...cur, [key]: !cur[key] } }; });
  }
  ...
  async function handleRefuse() {
    ...
    try {
      ...
      if (d.ok) { setFinalStatus("refuse"); setRefusalOpen(false); toast.success("Document refusé — l'artisan a été notifié"); }
  ...
  async function handleSign() {
    ...
    try {
      ...
      if (d.ok) { setFinalStatus("signe"); setSignOpen(false); setPaymentOpen(true); }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="text-center space-y-3 max-w-sm">
          ...
          <p className="text-sm text-gray-500">Ce lien de consultation a expiré. Contactez votre artisan pour en recevoir un nouveau.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="text-center space-y-2">
          ...
          <p className="font-semibold text-gray-700">Document introuvable</p>
  ...
  const { doc, doc_type, artisan, client, chantier, lignes } = data;
  const docLabel = DOC_LABELS[doc_type] ?? "Document";
  const montantTTC = Number(doc.montant_ht) * (1 + Number(doc.tva) / 100);
  const montantTVA = Number(doc.montant_ht) * (Number(doc.tva) / 100);
    ...
    <div className="min-h-screen bg-gray-100">
      {annotateMode && (
        <div className="sticky top-0 z-30 bg-blue-600 text-white px-4 py-2.5 flex items-center justify-between text-sm font-medium shadow">
          ...
          <button onClick={() => setAnnotateMode(false)} className="hover:opacity-80"><X className="w-4 h-4" /></button>
        </div>
      )}
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="px-6 pt-6 pb-5 border-b border-gray-100 flex justify-between items-start gap-4">
            ...
            <div className="text-right shrink-0">
              <p className="text-2xl font-bold text-blue-700 uppercase">{docLabel}</p>
              <p className="text-sm font-semibold mt-0.5">N° {doc.numero}</p>
              <p className="text-xs text-gray-500">Date : {fmtDate(doc.date ?? doc.created_at)}</p>
              {doc.date_validite && <p className="text-xs text-gray-500">Validité : {fmtDate(doc.date_validite)}</p>}
              <span className={`mt-1.5 inline-block text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${
                doc.statut === "signe" ? "bg-green-100 text-green-700" :
                doc.statut === "refuse" ? "bg-red-100 text-red-700" :
                "bg-blue-100 text-blue-700"
              }`}>
                {doc.statut === "signe" ? "Signé" : doc.statut === "refuse" ? "Refusé" : "En attente"}
              </span>
            </div>
          </div>
          ...
          {client && (
            <div className="px-6 py-4 border-b border-gray-100 bg-gray-50">
              ...
              <p className="text-sm font-medium text-gray-800">{[client.prenom, client.nom].filter(Boolean).join(" ")}</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                    ...
                    <Fragment key={l.id}>
                      {isNewSection && (
                        <tr className="bg-blue-50 border-t border-blue-100">
                          <td colSpan={annotateMode ? 5 : 4} className="px-4 py-1.5 text-xs font-bold text-blue-700 uppercase tracking-wider">{l.section_nom}</td>
                        </tr>
                      )}
                      <tr className={`border-b border-gray-100 transition-colors ${i % 2 === 0 ? "bg-white" : "bg-gray-50/60"} ${ann.circled ? "ring-2 ring-inset ring-orange-400 bg-orange-50/40" : ""}`}>
                        <td className={`px-4 py-2.5 ${ann.strikethrough ? "line-through text-gray-400" : "text-gray-700"}`}>
                          {l.designation}
                          {ann.comment && !annotateMode && <p className="mt-1 text-xs text-orange-600 italic">{ann.comment}</p>}
                          {annotateMode && (
                            <Input value={ann.comment ?? ""} onChange={e => setLineComment(l.id, e.target.value)} placeholder="Commentaire sur cette ligne…" className="mt-1.5 text-xs h-7 border-orange-200 focus:border-orange-400" />
                          )}
                        </td>
                        <td className="text-right px-3 py-2.5 text-gray-600 whitespace-nowrap">{l.quantite} {l.unite}</td>
                        <td className="text-right px-3 py-2.5 text-gray-600 whitespace-nowrap">{fmt(Number(l.prix_unitaire))}</td>
                        <td className="text-right px-3 py-2.5 font-semibold text-gray-800 whitespace-nowrap">{fmt(total)}</td>
                        {annotateMode && (
                          <td className="px-2 py-2.5">
                            <div className="flex gap-1 justify-end">
                              <button onClick={() => toggleAnn(l.id, "strikethrough")} title="Rayer" className={`p-1 rounded transition-colors ${ann.strikethrough ? "bg-red-100 text-red-600" : "text-gray-400 hover:text-red-500 hover:bg-red-50"}`}>
                                <Strikethrough className="w-3.5 h-3.5" />
                              </button>
                              <button onClick={() => toggleAnn(l.id, "circled")} title="Entourer" className={`p-1 rounded transition-colors ${ann.circled ? "bg-orange-100 text-orange-600" : "text-gray-400 hover:text-orange-500 hover:bg-orange-50"}`}>
                ...
                {lignes.length === 0 && (
                  <tr><td colSpan={annotateMode ? 5 : 4} className="px-4 py-8 text-center text-gray-400 italic text-sm">Aucune prestation renseignée</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <div className="px-6 py-4 border-t border-gray-100 flex justify-end">
            <div className="w-64 space-y-1 text-sm">
              <div className="flex justify-between text-gray-600"><span>Total HT</span><span>{fmt(Number(doc.montant_ht))}</span></div>
              <div className="flex justify-between text-gray-600"><span>TVA ({doc.tva} %)</span><span>{fmt(montantTVA)}</span></div>
              <div className="flex justify-between font-bold text-base bg-blue-700 text-white px-3 py-2 rounded-lg mt-1">
                <span>Net à payer TTC</span><span>{fmt(montantTTC)}</span>
              </div>
            </div>
          </div>

          {/* Commentaire global annotation */}
          {annotateMode && (
            <div className="px-6 py-4 border-t border-blue-100 bg-blue-50/40">
              ...
              <Textarea value={globalComment} onChange={e => setGlobalComment(e.target.value)} placeholder={`Ajoutez un commentaire global sur ce document…`} rows={3} className="mt-1.5 text-sm border-blue-200 focus:border-blue-400" />
              <div className="flex gap-2 mt-3 justify-end">
                <Button variant="outline" size="sm" onClick={() => setAnnotateMode(false)} className="text-xs">Annuler</Button>
        ...
        {finalStatus === "signe" && (
          <div className="bg-green-50 border border-green-200 rounded-xl px-5 py-4 flex items-center gap-3">
            ...
            <p className="text-sm text-green-700 font-medium">Document signé et validé — merci ! L'artisan a été notifié.</p>
          </div>
        )}
        {finalStatus === "refuse" && (
          <div className="bg-red-50 border border-red-200 rounded-xl px-5 py-4 flex items-center gap-3">
            <X className="w-5 h-5 text-red-600 shrink-0" />
            <p className="text-sm text-red-700 font-medium">Document refusé — l'artisan a été notifié.</p>
          </div>
        )}

        {!finalStatus && (
          <div className="grid grid-cols-3 gap-3">
            <Button variant="outline" onClick={() => setAnnotateMode(v => !v)} className={`flex flex-col items-center gap-1.5 h-auto py-3 text-xs border-2 transition-all ${annotateMode ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-200 text-gray-600 hover:border-blue-300 hover:text-blue-600"}`}>
              <Edit3 className="w-5 h-5" />
              Annoter
            </Button>
            <Button variant="outline" onClick={() => setRefusalOpen(true)} className="flex flex-col items-center gap-1.5 h-auto py-3 text-xs border-2 border-gray-200 text-gray-600 hover:border-red-300 hover:text-red-600 hover:bg-red-50 transition-all">
              <X className="w-5 h-5" />
              Refuser
            </Button>
            <Button onClick={() => setSignOpen(true)} className="flex flex-col items-center gap-1.5 h-auto py-3 text-xs bg-green-600 hover:bg-green-700 text-white border-2 border-transparent transition-all">
      ...
      <Dialog open={refusalOpen} onOpenChange={v => { if (!v) setRefusalOpen(false); }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-red-600"><X className="w-4 h-4" /> Refuser ce document</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <Label className="text-sm text-gray-600">Laissez un commentaire expliquant votre refus</Label>
            <Textarea value={refusalComment} onChange={e => setRefusalComment(e.target.value)} placeholder="Ex : Prix trop élevé, prestations non adaptées…" rows={4} className="resize-none" />
          </div>
          <DialogFooter>
      ...
      <Dialog open={signOpen} onOpenChange={v => { if (!v) setSignOpen(false); }}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-green-700"><Check className="w-4 h-4" /> Valider et signer</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Bon pour accord</Label>
              <Input value={bonPourAccord} onChange={e => setBonPourAccord(e.target.value)} className="text-sm" />
            </div>
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <Label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Votre signature</Label>
                <button onClick={clearCanvas} className="text-xs text-gray-400 hover:text-gray-600 flex items-center gap-1"><RotateCcw className="w-3 h-3" /> Effacer</button>
              </div>
              <canvas ref={canvasRef} className="w-full h-36 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50 touch-none cursor-crosshair"
            ...
            <div className="bg-gray-50 rounded-lg px-3 py-2 text-xs text-gray-500 flex justify-between">
              <span>Date de signature</span><span className="font-medium text-gray-700">{today}</span>
            </div>
            <p className="text-xs text-gray-400 italic">
              En validant, vous acceptez les conditions du document {doc?.numero} d'un montant de {fmt(montantTTC)} TTC.
            </p>
          </div>
          <DialogFooter>
      ...
      {/* Dialog : Confirmation après signature */}
      <Dialog open={paymentOpen} onOpenChange={v => { if (!v) setPaymentOpen(false); }}>
        <DialogContent className="max-w-md text-center">
          <div className="py-4 space-y-4">
            ...
            <DialogHeader>
              <DialogTitle className="text-center text-green-700">Document signé avec succès !</DialogTitle>
            </DialogHeader>
            ...
            <div className="bg-green-50 rounded-xl px-4 py-3 text-sm text-green-700">
              <p className="font-medium">Votre {docLabel.toLowerCase()} a bien été enregistré.</p>
              <p className="mt-1 text-green-600">L'artisan vous contactera prochainement.</p>
            </div>
          </div>
          <DialogFooter className="justify-center">
            <Button onClick={() => setPaymentOpen(false)} className="bg-green-600 hover:bg-green-700 text-white px-8">Fermer</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
